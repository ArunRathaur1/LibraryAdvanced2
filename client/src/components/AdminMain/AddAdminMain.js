...
const AddAdminMain = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const token = Cookies.get('authTokenmain'); // Extract token from cookie
    try {
      const response = await fetch('http://localhost:5000/library/add-admin-main', {
        method: 'POST',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      ...
      console.log(token);
      try {
        const response = await fetch(`http://localhost:5000/library/delete-admin-main`, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `${token}`, // Send token in Authorization header
        ...
        if (response.ok) {
          ...
          setFormData({ username: '', password: '' }); // Reset fields after deletion
        } else {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-50 to-blue-100 "style={{paddingTop:'100px',paddingBottom:'200px'}}>
      <div className="p-6 max-w-lg mx-auto rounded shadow-md bg-white">
        <h1 className="text-2xl font-bold mb-6 text-center">Add/Delete Admin</h1>
